...
@click.option(
    "--migrate-istio-dashboards",
    help="If set, Monitoring V1 Istio dashboards will automatically be migrated. This flag will be ignored if a project-id is provided.",
    is_flag=True,
)
@click.option(
    "--migrate-default-dashboards",
    help="If set, Monitoring V1 default dashboards will automatically be migrated. This flag will be ignored if a project-id is provided.",
    is_flag=True,
)
def migrate(
    rancher_url,
    ...
    cluster_id,
    insecure,
    migrate_istio_dashboards,
    migrate_default_dashboards,
):
    verify = not insecure
    ...
    for project in projects["data"]:
        ...
        base_url = (
            "%s/k8s/clusters/%s/api/v1/namespaces/cattle-prometheus-%s/services/http:access-grafana:80/proxy"
            % (rancher_url, cluster_id, project_id)
        )
        configmap_list = configmap_list + get_dashboards(
            base_url,
            headers,
            verify,
            migrate_istio_dashboards,
    ...
    base_url = (
        "%s/k8s/clusters/%s/api/v1/namespaces/cattle-prometheus/services/http:access-grafana:80/proxy"
...
def get_dashboards(
    ...
    all_dashboards_response = requests.get(
        "%s/api/search" % base_url, headers=headers, verify=verify
    )
    ...
    for dashboard_ref in all_dashboards:
        dashboard_response = requests.get(
            "%s/api/dashboards/uid/%s" % (base_url, dashboard_ref["uid"]),
            headers=headers,
            verify=verify,
        )
        ...
        if (is_project or not migrate_istio_dashboards) and dashboard_spec[
            "title"
        ] in DefaultIstioDashboards:
            continue
        if (is_project or not migrate_default_dashboards) and dashboard_spec[
            "title"
        ] in DefaultClusterDashboards:
            continue
        if "tags" not in dashboard_spec:
            dashboard_spec["tags"] = []
        dashboard_spec["tags"].append("migrated")
        dashboard_spec["title"] = "V1/%s" % (dashboard_spec["title"])
        dashboard_json = json.dumps(dashboard_spec).replace(
            "RANCHER_MONITORING", "Prometheus"
        )
