...
def operator_exists(rancher_url, system_project_id, headers, verify):
    operator_deployment_url = (
        "%s/v3/project/%s/workloads/deployment:cattle-prometheus:prometheus-operator-monitoring-operator"
        % (rancher_url, system_project_id)
    )
    operator_deployment_response = requests.get(
        operator_deployment_url, headers=headers, verify=verify
    )
...
def check_if_monitoring_is_disabled(
    rancher_url, rancher_api_token, cluster_id, insecure
):
    verify = not insecure
    requests.packages.urllib3.disable_warnings()
    yaml.add_representer(OrderedDict, ordered_dict_presenter)

    headers = {"authorization": "Bearer %s" % rancher_api_token}
    ...
    if (
        "enableClusterMonitoring" in cluster
        and cluster["enableClusterMonitoring"] == True
    ):
    ...
    if cluster_monitoring_enabled or cluster_alerting_enabled:
        if (
            "clusterTemplateRevisionId" in cluster
            and cluster["clusterTemplateRevisionId"]
        ):
            ctrid_url = "%s/v3/clustertemplaterevisions/%s" % (
                rancher_url,
            ...
            if "clusterConfig" in ctrid:
                ...
                if (
                    cluster_config["enableClusterMonitoring"]
                    or cluster_config["enableClusterAlerting"]
                ):
                    print(
                        "The RKE template revision used to spin up the cluster enables Monitoring / Alerting V1. "
    ...
    for project in projects["data"]:
        ...
        if "authz.management.cattle.io/system-project" in project["labels"]:
            ...
            "enableProjectMonitoring" in project
            and project["enableProjectMonitoring"] == True
        ):
    ...
    if not operator_exists(rancher_url, system_project_id, headers, verify):
        print(
            "The Monitoring V1 operator does not appear to exist in cluster %s. Migration to Monitoring V2 should be possible."
            % cluster_id
        )
        return

    print(
        "Found Monitoring V1 operator in cluster %s. Auditing cluster to see why it is deployed...\n"
        % cluster_id
    )

    if not (
        cluster_monitoring_enabled
        or cluster_alerting_enabled
        or len(project_monitoring_enabled) > 0
    ):
        print(
    ...
    if len(project_monitoring_enabled) > 0:
        print(
            "\tproject.spec.enableProjectMonitoring=True for projects %s"
            % [p["name"] for p in project_monitoring_enabled]
        )
    ...
    if cluster_monitoring_enabled:
        print(
            "To set cluster.spec.enableClusterMonitoring=False, click on Disable at %s/c/%s/monitoring/cluster-setting."
            % (rancher_url, cluster_id)
        )
        print("")

    if len(project_monitoring_enabled) > 0:
        print(
            "To set project.spec.enableProjectMonitoring=False, click on Disable at the following URLs:"
        )
        for project in project_monitoring_enabled:
            print(
                "Project %s: %s/p/%s/monitoring/project-setting"
                % (project["name"], rancher_url, project["id"])
            )
        print("")
    ...
    if cluster_alerting_enabled:
        ...
        if len(notifiers["data"]) == 0:
            print(
                "cluster.spec.enableClusterAlerting should be set to False since no Notifiers exist. Please file a bug with Rancher at https://github.com/rancher/rancher/issues/new."
            )
            return
        ...
        print(
            "To set cluster.spec.enableClusterAlerting=False, delete all notifiers in %s/c/%s/notifiers."
            % (rancher_url, cluster_id)
        )
        print(
            "Alternatively, you can remove recipients from all of the following Alert Groups (note: you will need to enable Monitoring to remove them):"
        )
